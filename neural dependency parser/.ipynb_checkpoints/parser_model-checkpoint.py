...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_bias = nn.Parameter(nn.init.uniform_(torch.empty(hidden_size)))
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.tensor([])
        x = torch.cat(
        [torch.index_select(self.embeddings, dim=0, index=features).flatten().unsqueeze(0)
         for features in w],
        dim=0
        )
    ...
    def forward(self, w):
        ...
        hidden = embeddings.matmul(self.embed_to_hidden_weight) + self.embed_to_hidden_bias
        hidden = nn.functional.relu(hidden)
        logits = hidden.matmul(self.hidden_to_logits_weight) + self.hidden_to_logits_bias
