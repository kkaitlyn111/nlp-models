...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':
            ...
            head = self.stack[-2]
            self.dependencies.append((dependent, head))
            self.stack.pop(-2)
        else:
    ...
    def finished(self):
        """Determines if the parse is complete
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        unfinished_parses = [parse for parse in unfinished_parses if not parse.finished()]
