...
def precompute_rotary_emb(dim, max_positions):
    ...
    theta = 1.0 / (10000 ** (torch.arange(0, dim, 2) / dim)) # find theta_i for every dim
    for t in range(max_positions):
        angle = t * theta
        rope_cache[t, :, 0] = torch.cos(angle)  # cos(t * theta_i)
        rope_cache[t, :, 1] = torch.sin(angle)  # sin(t * theta_i)
    pass
    ### END YOUR CODE ###
    return rope_cache
...
def apply_rotary_emb(x, rope_cache):
    ...
    rope_cache = rope_cache[:seq_len]  # truncate rope_cache to seq len
    x_complex = torch.view_as_complex(x.reshape(batch_size, seq_len, dim // 2, 2)) # reshape into pairs of features
    rope_cache_complex = torch.view_as_complex(rope_cache)
    rotated_x_complex = x_complex * rope_cache_complex.unsqueeze(0)
    rotated_x = torch.view_as_real(rotated_x_complex)
    rotated_x = rotated_x.reshape(batch_size, seq_len, dim)
    pass
    ### END YOUR CODE ###
    return rotated_x
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.rope:
            ...
            rope_cache = None
            ### YOUR CODE HERE ###
            dim = config.n_embd // config.n_head  # dim per head
    ...
    def forward(self, x):
        ...
        if self.rope:
            ...
            ### YOUR CODE HERE ###
            q = apply_rotary_emb(q.transpose(1, 2), self.rope_cache).transpose(1, 2)
            k = apply_rotary_emb(k.transpose(1, 2), self.rope_cache).transpose(1, 2)
            pass
            ### END YOUR CODE ###
