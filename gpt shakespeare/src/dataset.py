...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        avg_mask_len = max(1, int(len(trunc_doc) // 4)) # at least 1 char
        mask_len = random.randint(1, min(avg_mask_len * 2, len(trunc_doc)))
        mask_start = random.randint(0, len(trunc_doc) - mask_len)  # pick random start index for masked_content
        prefix = trunc_doc[:mask_start]
        masked_content = trunc_doc[mask_start:mask_start + mask_len]
        suffix = trunc_doc[mask_start + mask_len:]
