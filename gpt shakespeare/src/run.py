...
if args.function == 'pretrain':
    ...
    training_dataset = dataset.NameDataset(pretrain_dataset, corpus) # set up data

    tconf = trainer.TrainerConfig(max_epochs=75, batch_size=256, learning_rate=args.finetune_lr, lr_decay=True, warmup_tokens=512*20, final_tokens=200*len(pretrain_dataset)*block_size, num_workers=4, writer=writer)  # without pretraining params

    if args.writing_params_path is not None: # case with pretraining
        tconf = trainer.TrainerConfig(max_epochs=10, batch_size=256, learning_rate=args.finetune_lr, lr_decay=True, warmup_tokens=512*20, final_tokens=200*len(pretrain_dataset)*block_size, num_workers=4, writer=writer)  # with pretraining params
        model.load_state_dict(torch.load(args.reading_params_path, weights_only=True))
    
    trainer = trainer.Trainer(model, training_dataset, None, tconf)
    trainer.train()

    torch.save(model.state_dict(), args.writing_params_path) # save model weights
    pass
    ### END YOUR CODE ###
